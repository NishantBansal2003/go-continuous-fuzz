...
func (c *Container) WaitAndGetLogs(ID, pkg, target string,
	...
	// If the context was canceled or timed out, do not send any results on
	// the channels.
	if c.ctx.Err() != nil {
		return
	}

	// If the log stream finishes before the context is cancelled, it means
	// either a fuzzing crash occurred or the container exited due to some
	// other reason. In either case, proceed to check and report the result
	// accordingly.
	...
	// Container exited: retrieve the container's exit
	// status.
	errChan <- c.Wait(ID)
}

...
type fuzzOutputProcessor struct {
	...
	// fuzzCrashChan stores information about any crash encountered during
	// fuzzing.
	fuzzCrashChan chan fuzzCrash
}

// NewFuzzOutputProcessor constructs a fuzzOutputProcessor for the given logger,
// config, corpus directory, and fuzz crash channel.
func NewFuzzOutputProcessor(logger *slog.Logger, cfg *Config, corpusDir string,
	fuzzCrashChan chan fuzzCrash) *fuzzOutputProcessor {

	return &fuzzOutputProcessor{
		logger:        logger,
		cfg:           cfg,
		corpusDir:     corpusDir,
		fuzzCrashChan: fuzzCrashChan,
	}
}
...
func (fp *fuzzOutputProcessor) processFuzzStream(stream io.Reader) (bool,
	...
	err := fp.processFailureLines(scanner)
...
func (fp *fuzzOutputProcessor) processFailureLines(scanner *bufio.Scanner,
	...
	for scanner.Scan() {
		...
		if err != nil {
			return fmt.Errorf("processing fuzz stream: %w", err)
		}
	}

	// Send all captured fuzz crash data to notify the caller.
	fp.fuzzCrashChan <- fuzzCrash{

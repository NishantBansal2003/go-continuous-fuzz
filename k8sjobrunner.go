...
func (k *K8sJob) Start() (string, error) {
	...
	_, err := k.clientset.BatchV1().Jobs(Namespace).Create(k.ctx, job, metav1.CreateOptions{})
	if err != nil {
...
func (k *K8sJob) WaitAndGetLogs(jobName, pkg, target string,
	...
	logsReq := k.clientset.CoreV1().Pods(Namespace).GetLogs(pod.Name,
...
func (k *K8sJob) waitForJobCompletion() error {
	...
	lw := &cache.ListWatch{
		ListFunc: func(opts metav1.ListOptions) (runtime.Object,
			...
			return k.clientset.BatchV1().Jobs(Namespace).
				List(k.ctx, opts)
		},
		WatchFunc: func(opts metav1.ListOptions) (watch.Interface,
			error) {
...
func (k *K8sJob) Stop(jobName string) {
	...
	err := k.clientset.BatchV1().Jobs(Namespace).Delete(
